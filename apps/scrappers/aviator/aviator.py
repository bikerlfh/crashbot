...
class Aviator(AbstractGameBase, abc.ABC):
    ...
    def read_multipliers(self):
        if not self._page or not self._history_game:
            ...
            # })
            raise Exception("readMultipliers :: the page or the history game not exists")
        items = self._history_game.locator('app-bubble-multiplier.payout.ng-star-inserted').all()
        items.reverse()
        for item in items:
    ...
    def wait_next_game(self):
        ...
        while True:
            ...
            last_multiplier = self._format_multiplier(last_multiplier_content) if last_multiplier_content else last_multiplier_saved
