...
class BetControl(AbstractControlBase):
    ...
    def set_auto_cash_out(self, multiplier, control):
        ...
        value = round(float(auto_cash_out_multiplier.input_value(timeout=1000)), 2)
        if value != multiplier:
            auto_cash_out_multiplier.fill("", timeout=1000)
            auto_cash_out_multiplier.type(str(multiplier), delay=100)
    ...
    def update_amount(self, amount, control):
        ...
        value = round(float(input_element.input_value(timeout=1000)), 0)
        if value != amount:
            input_element.fill("", timeout=1000)
            input_element.type(str(amount), delay=100)
    ...
    def bet(self, amount, multiplier, control):
        ...
        self.set_auto_cash_out(multiplier, control)
        self.update_amount(amount, control)
        ...
        if control == Control.Control1:
            self._bet_button_1.click(delay=self._random_delay())
            # self.aviator_page.wait_for_timeout(1000)
            return

        self._bet_button_2.click(delay=self._random_delay())
        # self.aviator_page.wait_for_timeout(1000)
